...
<body>
    レシピの投稿<br>
    <form method="post" action="update.php?id=<?= htmlspecialchars($result['id'], ENT_QUOTES, 'UTF-8') ?>">
        料理名：<input type="text" name="recipe_name" value="<?= htmlspecialchars($result['recipe_name'], ENT_QUOTES, 'UTF-8') ?>"><br>
        カテゴリ：
        ...
        <br>
        予算：<input type="number" name="budget" value="<?= htmlspecialchars($result['budget'], ENT_QUOTES, 'UTF-8') ?>">円
        <br>
        作り方：
        <textarea name="howto" cols="40" rows="4" maxlength="320"><?= htmlspecialchars($result['howto'], ENT_QUOTES, 'UTF-8') ?></textarea>
        <br>
